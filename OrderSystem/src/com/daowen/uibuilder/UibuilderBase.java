...
public  class UibuilderBase{
	...
	public String buildImageLanmu(List list,String lanmudes )
	{
		...
		for(Iterator iterator = list.iterator();iterator.hasNext();){
		  ...
		  sb.append(MessageFormat.format("<a href=\"{0}/e/{1}info.jsp?id={2}\">",SystemParam.getSiteRoot(), tablename,id));
		  sb.append(MessageFormat.format("<img src=\"{0}\"/> ",imagefieldvalue));
		  sb.append("</a>");
		  sb.append("\r\n");
		  sb.append(MessageFormat.format("<div class=\"text-info\"><a href=\"{0}/e/{1}info.jsp?id={2}\">{3}</a></div>",SystemParam.getSiteRoot(),tablename,id,texfieldvalue));
		  //end image-item
		  sb.append("</div>");
		...
		if(list!=null&&list.size()==0){
			...
			sb.append("<div style=\"padding:40px;\">暂无相关商品</div>");
		}
